...
import sys
# Add to path
sys.path.append('/content/content/gudhi-devel-tags-gudhi-release-3.2.0.rc1/build/src/python')
...
def orbit5k_eval(nTimes, rand_state, noise_prob_list, X_processed_file_list,
      X_grid_processed_file_list, y_file, model_mlp_file_array,
    ...
    for iNoise in range(nNoise):
        ...
        for iTime in range(nTimes):
            ...
            model_mlp.load_weights(
                  model_mlp_file_array[iNoise][iTime])
            ...
            accuracy_mlp_pllay[iTime] = model_mlp_pllay.evaluate(test_dataset)[1]
            print("--- %s seconds ---" % (time.time() - start_time_inside))
            ...
            model_cnn.load_weights(
                  model_cnn_file_array[iNoise][iTime])
            accuracy_cnn[iTime] = model_cnn.evaluate(test_dataset_grid)[1]
...
orbit5k_eval(nTimes=nTimes, rand_state=rand_state, 
